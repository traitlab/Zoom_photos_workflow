...
centroids <- read.csv("20241125_bci25haplot_m3e_rgb_gr0p07_infer_wptso.csv")

trees <- st_read("20241125_bci25haplot_m3e_rgb_gr0p07_infer.gpkg") %>% 
  mutate(fid = as.integer(rownames(.)))%>%
  rename(polygon_id=fid)

centroids_area <- merge(centroids, trees, by="polygon_id" )

over80 <- filter(centroids_area, centroids_area$area>80000)

over80_adjusted <- over80 %>%dplyr::select(index,
                                            polygon_id,
                                            cluster_id,
                                            distance_from_takeoff_point,
                                            lon_x,
...
write.csv(over80_adjusted,"bci25haplot_treesover80m2_wptso.csv")
